...
class MusicModel(LightningModule):
    ...
    Args:
        d_model: Dimension of model (required).
        data_len: Length of input data (required).
        dropout: Dropout rate (required).
        ff: Number of feed-forward layers (required).
    ...
    def training_step(self, batch: Tuple[Tensor, Tensor], *args,
                      **kwargs) -> Tensor:
        data, position = batch
        output = self(data[:, :-1], position[:, :-1])
        loss = self.loss(output, data[:, 1:])
        self.acc(output, data[:, 1:])
