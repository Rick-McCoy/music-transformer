...
class SimpleModel(LightningModule):
    ...
    def training_step(self, batch: Tuple[Tensor, Tensor], *args,
                      **kwargs) -> Tensor:
        data, label = batch
    ...
    def validation_epoch_end(self, outputs: List[Dict[str, Tensor]]) -> None:
        ...
        preds = torch.cat([output['pred'] for output in outputs], dim=0)
        self.logger.experiment[0].add_pr_curve("val_pred", labels, preds, 0)

    def test_step(self, batch: Tuple[Tensor, Tensor], batch_idx: int, *args,
                  **kwargs) -> Tensor:
        data, label = batch
        output = self.classifier(data)
        loss = self.loss(output, label)
    ...
    def test_epoch_end(self, outputs: List[Dict[str, Tensor]]) -> None:
        labels = torch.cat([output['label'] for output in outputs], dim=0)
        preds = torch.cat([output['pred'] for output in outputs], dim=0)
        self.logger.experiment[0].add_pr_curve("test_pred", labels, preds, 0)
